...
// Plugins e mapa de componentes vivem FORA do render, como constantes de módulo.
// Não é micro-otimização: definidos inline, cada render criava funções NOVAS para h1,
// p, code... e o React trata tipo de componente novo como outro componente — desmonta
// a árvore inteira e monta outra. O DOM era recriado do zero a cada re-render do pai,
// e o navegador perde a SELEÇÃO de texto quando os nós somem. Resultado: bastava
// apertar Ctrl (que re-renderizava o PreviewPanel) pra a seleção evaporar e o Ctrl+C
// não ter o que copiar. Com as constantes estáveis, o React só remenda o que mudou.
...
const COMPONENTS = {
  ...
  h2: ({ node, ...p }) => (
    <h2 className="mb-1.5 mt-3 text-[14.5px] font-semibold text-foreground first:mt-0" {...p} />
  ),
  h3: ({ node, ...p }) => (
    <h3 className="mb-1 mt-2.5 text-[13.5px] font-semibold text-primary first:mt-0" {...p} />
  ),
  h4: ({ node, ...p }) => (
...
// memo: o preview de markdown fica dentro do PreviewPanel, que re-renderiza por muitos
// motivos alheios ao texto. Sem o memo, cada um deles reprocessava o markdown inteiro.
export default memo(function Markdown({ text, className }) {
  return (
